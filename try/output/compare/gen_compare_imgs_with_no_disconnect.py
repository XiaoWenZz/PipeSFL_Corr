...
def plot_comparison(no_corr_file, corr_files, dp_zero_file, output_file):
    # 加载 no_corr 文件数据
    ...
    # 绘制 dp 为 0 的参照曲线
    if dp_zero_file:
        dp_zero_data = pd.read_csv(dp_zero_file, header=None).squeeze()
        plt.plot(epochs, dp_zero_data, label='dp = 0.00', linestyle='--')
...
for no_corr_file in no_corr_files:
    ...
    # 查找 dp 为 0 的文件
    dp_zero_file = None
    dp_zero_pattern = f'{client_or_server}_Acc_no_Corr_ep{ep_value}_dp0.00_dr{dr_value}'
    if le_value:
        dp_zero_pattern += f'_le{le_value}'
    for f in os.listdir(no_corr_folder):
        if f.startswith(dp_zero_pattern) and f.endswith('.csv'):
            dp_zero_file = os.path.join(no_corr_folder, f)
            break
    ...
    # 绘制对比图
    plot_comparison(no_corr_file_path, corr_files, dp_zero_file, output_file)
