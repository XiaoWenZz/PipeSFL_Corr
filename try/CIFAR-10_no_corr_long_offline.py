...
class Client(object):
    def __init__(self, net_client_model, idx, lr, net_glob_server, criterion, count1, idx_collect, num_users, running,
        ...
        self.heartbeat_thread = multiprocessing.Process(target=self.send_heartbeat)
...
if __name__ == '__main__':
        ...
        for idx in idxs_users:
            if idx in idx_disconnected:
                local = Client(net_glob_client, idx, lr, net_glob_server, criterion, count1, idx_collect, num_users,
                               ...
                               idx_disconnected=idx_disconnected, running=running, is_disconnected=True, idx_disconnected_time=idx_disconnected_time)
            else:
                local = Client(net_glob_client, idx, lr, net_glob_server, criterion, count1, idx_collect, num_users,
                               dataset_train=dataset_train,
                               dataset_test=dataset_test, idxs=dict_users[idx], idxs_test=dict_users_test[idx],
                               heartbeat_queue=heartbeat_queue, disconnect_prob=disconnect_prob,
                               idx_disconnected=idx_disconnected, running=running, is_disconnected=False, idx_disconnected_time=idx_disconnected_time)
    ...
    # 保存模型 命名为 模型名+当前时间
    client_model_filename = os.path.join(model_dir, f'Client_ep{args.epochs}_dp{args.disconnect_prob:.2f}_' + time.strftime("%Y%m%d-%H%M%S",
                                                                                                         time.localtime()) + '.pth')
    server_model_filename = os.path.join(model_dir, f'Server_ep{args.epochs}_dp{args.disconnect_prob:.2f}_' + time.strftime("%Y%m%d-%H%M%S",
                                                                                                         time.localtime()) + '.pth')
    ...
    acc_train_filename = os.path.join(acc_dir, f'Client_Acc_ep{args.epochs}_dp{args.disconnect_prob:.2f}_' + time.strftime("%Y%m%d-%H%M%S",
                                                                                                        time.localtime()) + '.csv')
    ...
    loss_train_filename = os.path.join(loss_dir,
                                       f'Client_Loss_ep{args.epochs}_dp{args.disconnect_prob:.2f}_' + time.strftime("%Y%m%d-%H%M%S",
                                                                                                 time.localtime()) + '.csv')
    loss_train_df.to_csv(loss_train_filename, index=False)
    # 命名为 模型名+ 数据名+当前时间 目录为 output/acc
    acc_test_filename = os.path.join(acc_dir, f'Server_Acc_ep{args.epochs}_dp{args.disconnect_prob:.2f}_' + time.strftime("%Y%m%d-%H%M%S",
                                                                                                       time.localtime()) + '.csv')
    acc_test_df.to_csv(acc_test_filename, index=False)
    # 命名为 模型名+ 数据名+当前时间 目录为 output/loss
    loss_test_filename = os.path.join(loss_dir,
                                      f'Server_Loss_ep{args.epochs}_dp{args.disconnect_prob:.2f}_' + time.strftime("%Y%m%d-%H%M%S",
                                                                                                time.localtime()) + '.csv')
