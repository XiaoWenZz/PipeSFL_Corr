...
def train_server(fx_client, y, l_epoch_count, l_epoch, idx, len_batch, net_glob_server, lr, criterion,
                 ...
                 idx_collect, num_users, w_glob_server_buffer, extra_time, lock):
    ...
    net = ResNet50_server_side(100)

    # device = next(net.parameters()).device
    # print(device)
    # print(w_glob_server_buffer[0].device())
    extra_start_time = time.time()
    net.load_state_dict(w_glob_server_buffer[0])
    net.to('cuda:0')

    with lock:
        extra_time.append(extra_time[0] + time.time() - extra_start_time)
        del extra_time[0]

    # net_glob_server.train()
    net.train()
    # optimizer_server = torch.optim.Adam(net_glob_server.parameters(), lr=lr)
    optimizer_server = torch.optim.Adam(net.parameters(), lr=lr)
    ...
    with lock:
        del w_glob_server_buffer[0]
        w_glob_server_buffer.append(net.state_dict())
...
class Client(object):
    def __init__(self, net_client_model, idx, lr, net_glob_server, criterion, count1, idx_collect, num_users, priority_queue,
                 time_queue, BP_time_queue, FF_time_queue, w_glob_server_buffer, extra_time, lock, dataset_train = None,
                 dataset_test = None, idxs = None, idxs_test = None):
        self.idx = idx
        # self.device = device
        self.lr = lr
        self.local_ep = 1
        ...
        self.BP_time_queue = BP_time_queue
        self.FF_time_queue = FF_time_queue
    ...
    def train(self, net):
        ...
        for iter in range(self.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                images, labels = images.to('cpu'), labels.to('cuda:0')
                ...
                with self.lock:
                    del self.FF_time_queue[self.idx]
                    self.FF_time_queue.insert(self.idx, time.time() - FF_start_time)
                    ...
                    if len(self.priority_queue) == 0 or len(self.priority_queue) == 1:
                        ...
                        self.time_queue.append(self.FF_time_queue[self.idx] + self.BP_time_queue[self.idx])
                    else:
                        for t in range(1, len(self.priority_queue)):
                            if self.FF_time_queue[self.idx] + self.BP_time_queue[self.idx] > self.time_queue[t]:
                                self.priority_queue.insert(t, self.idx)
                                self.time_queue.insert(t, self.FF_time_queue[self.idx] + self.BP_time_queue[self.idx])
                                break
                            elif t == len(self.priority_queue) - 1:
                                self.priority_queue.append(self.idx)
                                self.time_queue.append(self.FF_time_queue[self.idx] + self.BP_time_queue[self.idx])
                    # sum = sum + self.FF_time_queue[self.idx] + self.BP_time_queue[self.idx]
                    # print('client ', self.idx, ' :', sum/self.count1)
                ...
                while True:
                    if len(self.priority_queue) >= 1:
                        if self.priority_queue[0] == self.idx:
                            ...
                            print('client ', self.idx, ' :', self.count1, '/', len_batch, 'time:', time.time())
                            dfx = train_server(client_fx, labels, iter, self.local_ep, self.idx, len_batch, self.net_glob_server,
                                               ...
                                               self.num_users, self.w_glob_server_buffer, self.extra_time, self.lock)
                ...
                with self.lock:
                    del self.BP_time_queue[self.idx]
                    self.BP_time_queue.insert(self.idx, time.time() - BP_start_time)
...
def multiprocessing_train_and_test(local, idx, net_glob_client, net_glob_server, w_locals_client, w_glob_server_buffer):
    net_glob_client.to('cpu')
    # net_glob_server.to('cuda:0')
    # with torch.cuda.device(idx):
    # Training ------------------
    w_client = local.train(net=copy.deepcopy(net_glob_client).to('cpu'))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="PipeSFLV2")
    ...
    if torch.cuda.is_available():
        ...
        # ===================================================================
    program = "FP-SFLV2 ResNet50 on Cifar100"
    ...
    FF_time_queue = manager.list()
    BP_time_queue = manager.list()
    lock = manager.Lock()

    # Init all clients' BP time is 0
    for i in range(num_users):
        FF_time_queue.append(0)
        BP_time_queue.append(0)

    extra_time = manager.list()
    extra_time.append(0)
    ...
    #----------------------------------------------------------------
    dict_users = dataset_iid(dataset_train, 16)
    dict_users_test = dataset_iid(dataset_test, 16)
    ...
    for iter in range(epochs):
        ...
        w_glob_server_buffer.append(net_glob_server.state_dict())
        processes = []

        for idx in idxs_users:
            local = Client(net_glob_client, idx, lr, net_glob_server, criterion, count1, idx_collect, num_users, priority_queue,
                           time_queue, BP_time_queue, FF_time_queue, w_glob_server_buffer, extra_time, lock, dataset_train = dataset_train,
        ...
        net_glob_server.load_state_dict(w_glob_server_buffer[0])

        print("====================== AP-SFL V2 =========================")
        print('========== Train: Round {:3d} Time: {:2f}s ==============='.format(iter, time.time()-start_time-extra_time[0]))
        print("==========================================================")

        res = 'PipeSFLV2_ResNet50_Cifar100 Cluster ID 3 with {} clients: {:.4f}'.format(num_users, time.time()-start_time-extra_time[0]) + '\n'
        filename = '/data/backup_data/gyq/FP-SFL/AP-SFL/result/cluster6' + '/clients_'+ str(num_users) +'.txt'
