    ...
    def train(self, net):
        ...
        for iter in range(self.local_ep):
            ...
            for batch_idx, (images, labels) in enumerate(self.ldr_train):
                ...
                # Client idx's task is started in Server
...
if __name__ == '__main__':
    ...
    # ===================================================================
    program = "PipeSFLV1 ResNet50 on Cifar100 - Serial"
    print(f"---------{program}----------")
    ...
    net_glob_server = ResNet50_server_side(100)
    ...
    # 保存模型 命名为 模型名+当前时间
    torch.save(net_glob_client.state_dict(),
               'PipeSFLV1_ResNet50_Cifar100_Serial_Client' + time.strftime("%Y%m%d%H%M%S", time.localtime()) + '.pth')
    torch.save(net_glob_server.state_dict(),
               'PipeSFLV1_ResNet50_Cifar100_Serial_Server' + time.strftime("%Y%m%d%H%M%S", time.localtime()) + '.pth')
    ...
    # 命名为 模型名+ 数据名+当前时间
    acc_train_df.to_csv(
        'PipeSFLV1_ResNet50_Cifar100_Serial_Client_Acc' + time.strftime("%Y%m%d%H%M%S", time.localtime()) + '.csv',
        index=False)
    loss_train_df.to_csv(
        'PipeSFLV1_ResNet50_Cifar100_Serial_Client_Loss' + time.strftime("%Y%m%d%H%M%S", time.localtime()) + '.csv',
        index=False)
    acc_test_df.to_csv(
        'PipeSFLV1_ResNet50_Cifar100_Serial_Server_Acc' + time.strftime("%Y%m%d%H%M%S", time.localtime()) + '.csv',
        index=False)
    loss_test_df.to_csv(
        'PipeSFLV1_ResNet50_Cifar100_Serial_Server_Loss' + time.strftime("%Y%m%d%H%M%S", time.localtime()) + '.csv',
        index=False)
