...
if __name__ == '__main__':
    ...
    curve_filename = os.path.join(curve_dir,
                                  'train_time_curve_no_Corr_ep{args.epochs}_dp{args.disconnect_prob:.2f}_' + time.strftime("%Y%m%d%H%M%S", time.localtime()) + '.png')
    plt.savefig(curve_filename)
    plt.clf()  # 清除当前图形

    # 保存模型 命名为 模型名+当前时间
    client_model_filename = os.path.join(model_dir, 'PipeSFLV1_no_Corr_ResNet50_HAM10000_Client_ep{args.epochs}_dp{args.disconnect_prob:.2f}_' + time.strftime("%Y%m%d%H%M%S",
                                                                                                         time.localtime()) + '.pth')
    server_model_filename = os.path.join(model_dir, 'PipeSFLV1_no_Corr_ResNet50_HAM10000_Server_ep{args.epochs}_dp{args.disconnect_prob:.2f}_' + time.strftime("%Y%m%d%H%M%S",
                                                                                                         time.localtime()) + '.pth')
    ...
    loss_train_filename = os.path.join(loss_dir,
                                       'PipeSFLV1_no_Corr_ResNet50_HAM10000_Client_Loss_ep{args.epochs}_dp{args.disconnect_prob:.2f}_' + time.strftime("%Y%m%d%H%M%S",
                                                                                                 time.localtime()) + '.csv')
    loss_train_df.to_csv(loss_train_filename, index=False)
    # 命名为 模型名+ 数据名+当前时间 目录为 output/acc
    acc_test_filename = os.path.join(acc_dir, 'PipeSFLV1_no_Corr_ResNet50_HAM10000_Server_Acc_ep{args.epochs}_dp{args.disconnect_prob:.2f}_' + time.strftime("%Y%m%d%H%M%S",
                                                                                                       time.localtime()) + '.csv')
    acc_test_df.to_csv(acc_test_filename, index=False)
    # 命名为 模型名+ 数据名+当前时间 目录为 output/loss
    loss_test_filename = os.path.join(loss_dir,
                                      'PipeSFLV1_no_Corr_ResNet50_HAM10000_Server_Loss_ep{args.epochs}_dp{args.disconnect_prob:.2f}_' + time.strftime("%Y%m%d%H%M%S",
                                                                                                time.localtime()) + '.csv')
    ...
    acc_curve_filename = os.path.join(curve_dir,
                                      'acc_curve_no_Corr_ep{args.epochs}_dp{args.disconnect_prob:.2f}_' + time.strftime("%Y%m%d%H%M%S", time.localtime()) + '.png')
    ...
    loss_curve_filename = os.path.join(curve_dir,
                                       'loss_curve_no_Corr_ep{args.epochs}_dp{args.disconnect_prob:.2f}_' + time.strftime("%Y%m%d%H%M%S", time.localtime()) + '.png')
